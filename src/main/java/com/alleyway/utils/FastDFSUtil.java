...
public class FastDFSUtil {
     ...
     * @param size 文件大小。会根据文件大小自动关闭流，因为我自动获取，所以这个参数取消了
     * @param fileName 文件名
     * @return
     */
    public String uploadFileStream(InputStream inputStream, String fileName) {
        long fileLength = 0;
        try {
	  fileLength =inputStream.available();//获取文件大小
        } catch (IOException e) {
	  e.printStackTrace();
        }
        StorePath path = storageClient.uploadFile(inputStream, fileLength, fileName, null);
        return getResAccessUrl(path);
    }
    ...
    /**
     * 返回文件上传成功后的地址名称ַ  M00/00/00/wKgASlzk3kGAFVF7AABeEJ_iOs4683.png
     * @param storePath
     * @return
     */
